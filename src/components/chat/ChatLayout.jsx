...
// ✅ FIX: Import the modern self-contained Chat component (Chat.jsx), NOT ChatWindow.jsx
// Chat.jsx manages its own socket, fetching, block status, reactions etc.
...
export default function ChatLayout({ partnerId, onClose }) {
  ...
  // 1. ADD THIS STATE FOR THE SOCKET
  const [socket, setSocket] = useState(null);
  // 2. ADD THIS EFFECT TO CONNECT TO THE SOCKET
  useEffect(() => {
    const newSocket = io(API);
    setSocket(newSocket);

    // Clean up connection when the layout closes
  ...
  // ✅ FIX: Also re-select partner if the prop changes (e.g. clicking "Contact" on a different donor)
  useEffect(() => {
      ...
      <div className="bg-white w-full md:w-[450px] h-screen flex flex-col shadow-[0_0_50px_rgba(0,0,0,0.1)] animate-in slide-in-from-right duration-300">
        ...
        <div className="flex-1 overflow-hidden flex flex-col bg-slate-50/30">
          {!selectedPartner ? (
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-3">
              {conversations.length === 0 ? (
                ...
                conversations.map((conv, index) => (
                  <button
                    key={conv._id || conv.otherUser?._id || index}
            ...
            /* ✅ FIX: Render Chat.jsx with the correct prop name: otherUserId */
